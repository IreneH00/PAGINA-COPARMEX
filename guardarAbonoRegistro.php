<?php
include 'conexion.php';

session_start();

if (isset($_POST['id'], $_POST['totalAnualidad'], $_POST['porCobrar'], $_POST['monto'], $_POST['formaPago'], $_POST['fechaPago'])) {
    
    $registro_evento_id = (int) $_POST['id'];
    $total_anualidad = (float) $_POST['totalAnualidad'];
    $monto = (float) $_POST['monto'];
    $forma_pago = $conex->real_escape_string($_POST['formaPago']);
    $fecha_pago = $conex->real_escape_string($_POST['fechaPago']);
    $creado_por = $_SESSION['nombre'] ?? 'Desconocido'; 

    
    $nuevo_por_cobrar = $total_anualidad - $monto;

   
    $query = "INSERT INTO abonos_eventos (registro_evento_id, total_anualidad, por_cobrar, monto, forma_pago, fecha_pago, creado_por)
              VALUES (?, ?, ?, ?, ?, ?, ?)";

    if ($stmt = $conex->prepare($query)) {
        $stmt->bind_param('idddsss', $registro_evento_id, $total_anualidad, $nuevo_por_cobrar, $monto, $forma_pago, $fecha_pago, $creado_por); 

        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'nuevo_por_cobrar' => $nuevo_por_cobrar]);
        } else {
            echo json_encode(['success' => false, 'message' => 'Error al guardar el abono.']);
        }

        $stmt->close();
    } else {
        echo json_encode(['success' => false, 'message' => 'Error al preparar la consulta.']);
    }
    
    $conex->close();
} else {
    echo json_encode(['success' => false, 'message' => 'Datos incompletos.']);
}
?>
