...
if (isset($_POST['id'], $_POST['totalAnualidad'], $_POST['porCobrar'], $_POST['monto'], $_POST['formaPago'], $_POST['fechaPago'])) {
    ...
    $query = "INSERT INTO abonos (socio_id, total_anualidad, por_cobrar, monto, forma_pago, fecha_pago)
              VALUES (?, ?, ?, ?, ?, ?)";

    
    if ($stmt = $conex->prepare($query)) {
       
        $stmt->bind_param('idddss', $socio_id, $total_anualidad, $por_cobrar, $monto, $forma_pago, $fecha_pago);
